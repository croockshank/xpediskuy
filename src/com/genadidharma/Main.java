...
public class Main {
    ...
    private static String[] provinces = {
            "Bali", "Bangka Belitung", "Gorontalo", "Jakarta", "Jawa Timur"
    };
    ...
    public static void main(String[] args) {
        ...
        double distance;
        int typeIndex, packagingIndex, serviceIndex;
        float weight, dimension;
        float packagingPrice;
        int subTotal, total, money;
        ...
        typeIndex = getTypeIndex(input);

        //Masukkan Berat Barang
        weight = getWeight(input);

        //Masukkan dimensi barang
        dimension = getDimension(input, typeIndex);

        //Pilih / tentukan packaging
        packagingIndex = getPackagingIndex(input, typeIndex, dimension);
        packagingPrice = roundToNearestHundred((int) calculatePackagingPrice(surfaceArea, packagingIndex));

        //Hitung total biaya sebelum biaya pelayanan
        subTotal = dimension > 0 ? (int) Math.round(distance) * (int) Math.ceil(weight) * (int) dimension : (int) Math.round(distance) * (int) Math.ceil(weight);
        ...
        printReceipt(distance, dimension, weight, packagingPrice, subTotal, total,  originProvinceIndex, originCityOrRegencyIndex, originDistrictIndex, destinationProvinceIndex, destinationCityOrRegencyIndex, destinationDistrictIndex, serviceIndex, packagingIndex);
    ...
    private static float getDimension(Scanner input, int typeIndex) {
            ...
            if (selected == 'y') {
                do {
                    if (checkInputValid("Masukkan panjang(cm): ", input, "float")) {
                        ...
                        if (length < 0) System.out.println("Masukkan angka yang sesuai");
                    }
                } while (length < 0);
                do {
                    if (checkInputValid("Masukkan lebar(cm): ", input, "float")) {
                        width = input.nextFloat();
                        if (width < 0) System.out.println("Masukkan angka yang sesuai");
                    }
                } while (width < 0);
                do {
                    if (checkInputValid("Masukkan tinggi(cm): ", input, "float")) {
                        height = input.nextFloat();
                        if (height < 0) System.out.println("Masukkan angka yang sesuai");
                    }
                } while (height < 0);
                dimension = length * width * height / 6000;
            }
        }
        surfaceArea = calculatePackageSurfaceArea(length, width, height);
    ...
    private static float calculatePackagingPrice(float surfaceArea, int packagingIndex) {
        return surfaceArea * packagingPrice[packagingIndex];
    }

    private static int getPackagingIndex(Scanner input, float typeIndex, float dimension) {
        int index = 0;
        if (typeIndex > 0 && dimension <= 5) {
            ...
            do {
                if (checkInputValid("Masukkan pilihan: ", input, "int")) {
                    index = input.nextInt();
                    if (index < 1) System.out.println("Masukkan pilihan yang sesuai!");
                }
            } while (index < 1 || index > packaging.length);
    ...
    private static void printReceipt(double distance, float dimension, float weight, float packagingPrice, long subTotal, int total, int... indexes) {
